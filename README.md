...
| 环形缓冲区 | 16384 条容量的无锁环形缓冲区，用于线程间数据传递 |
| 流控机制 | 当缓冲区使用率超过 75% 时自动跳过写入，防止溢出 |
| UI 批量更新优化 | 禁用重绘、LB_RESETCONTENT 等优化，避免 O(n²) 性能问题 |
| 调试日志 | 可配置的调试日志开关，默认关闭 |
...
| `EnableDebugLog()` | 开启或关闭调试日志（默认关闭） |

## 🔧 技术实现

### 环形缓冲区与流控

为实现扫描线程与 UI 线程之间的高效数据传递，采用 **无锁环形缓冲区 (Lock-free Ring Buffer)** 设计：

| 参数 | 值 | 说明 |
|-----|-----|------|
| 缓冲区大小 | 16384 条 | 可容纳 16384 个扫描结果条目 |
| 流控阈值 | 75% | 当缓冲区使用率超过此值时触发流控 |
| 内存序 | `memory_order_release` / `memory_order_acquire` | 确保数据跨线程可见 |

**流控机制**：当写入位置与读取位置的差值超过缓冲区大小的 75% 时，后续的扫描结果写入会被跳过，防止尚未消费的数据被覆盖，同时统计丢弃条目数量。

**UI 更新优化**：
- 禁用 ListBox 重绘（`WM_SETREDRAW`）期间批量重建列表
- 使用 `LB_RESETCONTENT` 清空列表，避免 O(n²) 的增量删除性能问题
- 限制显示条目为最近的 1000 条，防止列表过长导致的 UI 卡顿
